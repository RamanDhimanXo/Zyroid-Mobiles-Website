<?php
    ...
    if(isset($_POST['sbt'])){
        ...
        if(!preg_match("/^[a-zA-Z\s]+$/", $name)){
            ...
            $_SESSION['error'] = 'Phone must be 10-15 digits';
            $error = true;
        } elseif(!preg_match("/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[\W_]).{8,}$/", $raw_pass)){
            $_SESSION['error'] = 'Weak Password: Use 8+ chars with mix of types';
            $error = true;
        } elseif($raw_pass !== $raw_con_pass){
        ...
        if($error){
            ...
            $stmt = $con->prepare("SELECT email FROM tb_admin WHERE email = ?");
            ...
            if($stmt->get_result()->num_rows > 0){
                ...
                $stmt = $con->prepare("INSERT INTO tb_admin (fullname, email, phone, pass) VALUES (?, ?, ?, ?)");
...
<head>
    ...
    <title>Register - Zyroid Admin</title>
...
<body>
    ...
    <div class="admin-card" id="adminCard">
        ...
        <div class="text-center mb-5">
            <h3 class="fw-bold text-white mb-1">Create Admin Identity</h3>
            <p class="text-white-50 small">Register new administrative personnel</p>
        </div>
        ...
        <form id="regForm" method="POST" novalidate>
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    <div class="form-floating">
                        <input type="password" class="form-control" id="con_pass" name="con_pass" placeholder="Confirm" required>
        ...
        <div class="mt-4 text-center">
            <p class="text-white-50 small">Already have credentials? 
                <a href="index.php" class="text-success text-decoration-none fw-bold hover-link">Login Here</a>
            </p>
        </div>
    </div>
